...
export default function Rastreamento() {
    ...
    return (
        <div className="flex flex-col min-h-[calc(100vh-120px)] lg:h-[calc(100vh-140px)] gap-6 animate-fade-in relative z-0">
            <div className="flex justify-between items-start md:items-center flex-col md:flex-row gap-4">
                <div>
                    <h2 className="text-xl md:text-2xl font-bold font-mono tracking-tight">Rastreamento Global</h2>
                    <p className="text-muted text-xs md:text-sm mt-1">Conectado a {equipamentos.length} hardwares em campo.</p>
                </div>

                <div className="flex items-center gap-2 flex-wrap">
                    <button onClick={gerarExemplos} className="btn-premium flex items-center gap-2 text-[10px] md:text-xs py-1.5 px-3">
                        <PlusCircle size={14} /> Exemplos
                    </button>
                    <div className="flex items-center gap-2 bg-surface border border-border px-3 py-1.5 rounded-full text-[10px] md:text-xs font-mono">
                        <span className={`w-2 h-2 rounded-full ${wsStatus === 'Conectado' ? 'bg-success animate-pulse' : 'bg-danger'}`}></span>
                        <span className="text-muted hidden sm:inline">WebSocket:</span>
                        <span className={wsStatus === 'Conectado' ? 'text-success font-bold' : 'text-danger font-bold'}>{wsStatus}</span>
                    </div>
                </div>
            </div>

            <div className="flex-1 bg-surface border border-border rounded-xl shadow-glow overflow-hidden relative z-0 flex flex-col lg:flex-row">
                {/* LEAFLET CONTAINER */}
                <div className="relative flex-1 min-h-[300px] md:min-h-[400px] lg:h-full">
                    <MapContainer
                ...
                {/* FLOATING LIST / OVERLAY LIST (visible below map on mobile, sidebar on desktop) */}
                <div className="w-full lg:w-[320px] bg-bg-base/90 backdrop-blur border-t lg:border-t-0 lg:border-l border-border flex flex-col z-[400] h-[300px] lg:h-full">
                    <div className="p-4 border-b border-border bg-surface/50 sticky top-0 z-10">
                        <div className="flex justify-between items-center mb-3">
                            <h3 className="font-mono text-[10px] md:text-sm tracking-widest text-muted uppercase">Frota Ativa</h3>
                            <span className="text-[10px] font-bold bg-elevated px-2 py-1 rounded text-primary">{filteredEquipments.length} dispositivos</span>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
                        {filteredEquipments.length === 0 ? (
                            <div className="text-center py-10 text-xs text-muted">Nenhum hardware encontrado.</div>
                        ) : (
                            filteredEquipments.map(eq => (
                                <div
                                    key={eq.id}
                                ...
                                >
                                    ...
                                    <div className="grid grid-cols-2 gap-1 text-[10px] md:text-xs text-muted font-mono mt-3 pt-3 border-t border-border/50">
                                        <div>Vel: {eq.velocidade} km/h</div>
                                        <div>Bat: {eq.bateria}%</div>
                                        <div className="col-span-2 pt-1">Atualizado: {eq.ultimaAtt}</div>
                                    </div>
                                </div>
                            ))
                        )}
